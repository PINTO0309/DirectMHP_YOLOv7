...
class DirectMHPONNX(object):
    def __init__(
        self,
        model_file_path: Optional[str] = 'directmhp_cmu_yolov7_tiny_post_480x640.onnx',
        class_score_th: Optional[float] = 0.20,
        cpu_mode: Optional[bool] = False,
        cuda_mode: Optional[bool] = False,
        tensorrt_mode: Optional[bool] = False,
...
def main(args):
    ...
    cuda_mode: bool = args.cuda
    tensorrt_mode: bool = args.tensorrt
    # DirectMHP
    directmhp_head = \
        DirectMHPONNX(
    ...
    while True:
        ...
        key = cv2.waitKey(1) if '.png' not in str(args.device) and '.jpg' not in str(args.device) else cv2.waitKey(0)
        if key == 27:  # ESC
            print('')
            break
