...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        if nc == 1:
            x[:, 5:-num_angles] = x[:, 4:5] # for models with one class, cls_loss is 0 and cls_conf is always 0.5,
                                 # so there is no need to multiplicate.
        else:
            x[:, 5:-num_angles] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            ...
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(), x[i, -num_angles:]), 1)
        else:  # best class only
            conf, j = x[:, 5:-num_angles].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), x[:, -num_angles:]), 1)[conf.view(-1) > conf_thres]
