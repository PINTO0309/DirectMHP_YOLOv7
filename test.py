...
def test(
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # Compute loss
            if compute_loss:
                loss += compute_loss([x.float() for x in train_out], targets)[1][:4]  # box, obj, cls, mse

            # Run NMS
            targets[:, 2:6] *= torch.Tensor([width, height, width, height]).to(device)  # to pixels
            lb = [targets[targets[:, 0] == i, 1:] for i in range(nb)] if save_hybrid else []  # for autolabelling
            t = time_synchronized()
            out = non_max_suppression(out, conf_thres=conf_thres, iou_thres=iou_thres, labels=lb, multi_label=True, num_angles=data['num_angles'])
    ...
    if task in ('train', 'val'):
        ...
        if "AFLW2000" in annot or "300W_LP" in annot or "BIWI" in annot:
            total_num, pose_matrix, left_num, pose_matrix_b, left_num_b, pose_matrix_f, left_num_f = \
                mean_absolute_error_calculate_single(annot, jdict, frontal_face)
                # mean_absolute_error_calculate_single(annot, json_path, frontal_face)
        else:
            total_num, pose_matrix, left_num, pose_matrix_b, left_num_b, pose_matrix_f, left_num_f = \
                mean_absolute_error_calculate_v2(annot, jdict, frontal_face)
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()), maps, t, error_list
